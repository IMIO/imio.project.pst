...
class TestCreateSubActionLink(FunctionalTestCase):
    ...
    def test_scenarios_as_admin_in_pst_action_created(self, browser):
        api.content.transition(obj=self.a13, transition='back_to_created')
        state = api.content.get_state(obj=self.a13)
        self.assertEqual(state, 'created')
        self.call_scenarios(browser, self.pst_admin, self.a13)
    ...
    def test_scenarios_as_admin_in_pst_action_ongoing(self, browser):
        api.content.transition(obj=self.a13, transition='begin')
        state = api.content.get_state(obj=self.a13)
    ...
    def test_scenarios_as_admin_in_pst_action_stopped(self, browser):
        api.content.transition(obj=self.a13, transition='stop')
        state = api.content.get_state(obj=self.a13)
    ...
    def test_scenarios_as_admin_in_pst_action_terminated(self, browser):
        api.content.transition(obj=self.a13, transition='begin')
        api.content.transition(obj=self.a13, transition='finish')
        state = api.content.get_state(obj=self.a13)
    ...
    def test_scenarios_as_pst_editor_in_pst_action_to_be_scheduled(self, browser):
        state = api.content.get_state(obj=self.a13)
        self.assertEqual(state, 'to_be_scheduled')
        self.call_scenarios(browser, self.pst_editor, self.a13)
    ...
    def test_scenarios_as_manager_in_pst_action_created(self, browser):
        ...
        state = api.content.get_state(obj=self.a13)
        self.assertEqual(state, 'created')
        self.call_scenarios(browser, self.manager, self.a13)
    ...
    def step_3(self, browser):
        ...
        browser.find('Lien symbolique').fill(self.sa17)
    ...
    def step_4(self, browser):
        ...
        sa17l1 = self.a13['realiser-un-audit-energetique-du-batiment']
        self.assertNotEqual(self.sa17.UID(), sa17l1.UID())
