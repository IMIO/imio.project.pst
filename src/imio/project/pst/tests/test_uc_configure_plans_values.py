...
def step_3(browser):
    ...
    browser.find(u'Plans possibles').fill([{u'Libellé': 'Plan logement', u'Clé': 'plan-logement'}])
...
class TestConfigurePlansValues(FunctionalTestCase):
    """Use case tests.
    Name: Configure plans values
    Actor(s): pst admin
    Goal: allows actors to configure plans values
    Author: Franck Ngaha
    Created: 11/05/2021
    Updated: 11/05/2021
    ...
    def setUp(self):
        super(TestConfigurePlansValues, self).setUp()
        # Actors
    ...
    def main_scenario(self, browser, actor, context):
        ...
        step_3(browser)  # The actor update plans values field and save
