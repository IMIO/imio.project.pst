...
class TestAction(IntegrationTestCase):
    ...
    def test_find_deadlines_on_pst_sub_action_children(self):
        """Test find_deadlines_on_children method on pstsubaction."""
        self.assertEqual(find_deadlines_on_children(self.sa_17, {"task": "due_date"}),
    ...
    def test_find_max_deadline_on_pst_sub_action_children(self):
        """Test find_max_deadline_on_children method on pstsubaction."""
        self.assertEqual(find_max_deadline_on_children(self.sa_17, {"task": "due_date"}),
                         datetime.date(datetime(2020, 6, 30)))

    def test_find_brains_on_pst_action_parents(self):
        """Test find_brains_on_parents method on pstaction."""
        self.assertEqual([containers_brains[0].Title for containers_brains in find_brains_on_parents(self.a_16)], [
            "R\xc3\xa9duire la consommation \xc3\xa9nerg\xc3\xa9tique des b\xc3\xa2timents communaux de 20% d'ici 2024 "
            "(OO.15)", "Etre une commune qui s'inscrit dans la lign\xc3\xa9e des accords de r\xc3\xa9ductions des gaz "
    ...
    def test_find_brains_on_pst_sub_action_parents(self):
        """Test find_brains_on_parents method on pstsubaction."""
        self.assertEqual([containers_brains[0].Title for containers_brains in find_brains_on_parents(self.sa_17)], [
            ...
            "2024 (OO.15)", "Etre une commune qui s'inscrit dans la lign\xc3\xa9e des accords de r\xc3\xa9ductions des "
                            "gaz \xc3\xa0 effet de serre afin d'assurer le d\xc3\xa9veloppement durable (OS.10)"])
    ...
    def test_find_deadlines_on_pst_sub_action_parents(self):
        """Test find_deadlines_on_parents method on pstaction."""
        self.assertEqual(
            find_deadlines_on_parents(self.sa_17,
    ...
    def test_find_max_deadline_on_pst_sub_action_parents(self):
        """Test find_max_deadline_on_parents method on pstaction."""
        self.assertEqual(
            find_max_deadline_on_parents(self.sa_17,
                                         {"pstaction": "planned_end_date", "operationalobjective": "planned_end_date"}),
            datetime.date(datetime(2024, 12, 31)))
